...
for file in os.listdir():
    if file.endswith(".jpg"):
        ...
        if img is not None:
            ...
            if des is not None: learned_items[name] = {"kp": kp, "des": des}

def tts_worker():
    while True:
        text = tts_queue.get()
        if text: subprocess.run(['say', text]) 
        tts_queue.task_done()
...
def draw_skeleton(frame, kps):
    ...
    for kp in kps:
        if kp[2] > 0.5: cv2.circle(frame, (int(kp[0]), int(kp[1])), 4, (0,0,255), -1)
    ...
    while True:
        ...
        if des_f is not None:
            for name, data in learned_items.items():
                ...
                if len(matches) > 45: learned_alert = name
        ...
        for r in custom_results:
            for box in r.boxes:
                ...
                if lbl in ['dining table', 'desk', 'bed', 'chair', 'couch']:
                    if lbl in ['dining table', 'desk']: table_box = c
        ...
        for r in pose_results:
            for i, box in enumerate(r.boxes):
                if pose_brain.names[int(box.cls[0])] == 'person':
                    ...
                    if w > (h * 1.3): actions.append("fallen")
                    
                    if r.keypoints is not None:
                        kps = r.keypoints.data[i].cpu().numpy()
                        draw_skeleton(frame, kps)
                        if abs(kps[11][1] - kps[13][1]) < h * 0.15: actions.append("sitting")
                        if (kps[9][1] < kps[5][1]) or (kps[10][1] < kps[6][1]): actions.append("waving")
        ...
        if (now - last_summary_time) > SPEECH_DELAY:
            parts = []
            if learned_alert: parts.append(f"I found your {learned_alert}!")
            ...
            if table_items: parts.append(f"a table with a {table_items[0]} on it")
